...
T = 0.65*3000       # Rescaled time (3000 was the original final time)
num_steps = 1950    # Number of time steps 
dt = T / num_steps  # Time step size

# Create mesh and define function space
nx = ny = 100
mesh = RectangleMesh(Point(0, 0), Point(360, 360), nx, ny)
...
bp = Constant(-3.98/1.89 * pow(10,-1) )
cp = Constant(2.25/1.89 * pow(10,-2) )
eps = Constant(pow(10,-5)/ (1.89**2) ) 
...
class InitialConditions(UserExpression):
    ...
    def eval(self, values, x):
         W_unif = 1.123*pow(10,-9) * 6*pow(10,7) # Initial worm density at the uniform steady state
        # Perturb the uniform steady state:
        values[0] = W_unif + pow(10,-2)*(0.5 - random.random())
        # Equilibrium for oxygen:
        values[1] = Oam - values[0]
...
def Dw(u1,u2):
    return 0.5*(u2**2 + bp*u2 + cp)**2 

def beta(u1,u2):
    return 0.5*(u2**2 + bp*u2 + cp)*(2*u2 + bp)


# Define variational problem
L = eps*(u1 - u_n1) / dt * v_1 * dx \
+ Dw(u1,u2) * dot(grad(u1), grad(v_1)) * dx \
+ beta(u1,u2) * u1 * dot(grad(u2), grad(v_1)) * dx \
+ (u2 - u_n2) / dt * v_2 * dx \
+ dot(grad(u2), grad(v_2)) * dx \
- (Oam - u2) * v_2 * dx + u1 * v_2 * dx
...
vtkfile_u1 = File('u1_ND/u1.pvd')
vtkfile_u2 = File('u2_ND/u2.pvd')
