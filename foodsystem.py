...
d_w = Constant(pow(10,-12))     # Food depletion rate
Df = Constant(4 * pow(10,-12))  # Food diffusion rate
q = Constant(2.23*pow(10,-4))   # Speed after food depletion, set at V(0.21)
...
def Dw(u1,u2,u3):
    return ( u3*(a*u2**2 + b*u2 + c) + (1-u3)*q )**2 

def beta(u1,u2,u3): # with pd wrt O
    return (u3*(a*u2**2 + b*u2 + c) + (1-u3)*q) * u3 * (2*a*u2 + b)

def phi(u1,u2,u3): # with pd wrt F
    return (u3*(a*u2**2 + b*u2 + c) + (1-u3)*q) * (a*u2**2 + b*u2 + c - q)

# Define variational problem
L = (u1 - u_n1) / dt * v_1 * dx \
+ Dw(u1,u2,u3) * dot(grad(u1), grad(v_1)) * dx \
+ beta(u1,u2,u3) * u1 * dot(grad(u2), grad(v_1)) * dx \
+ phi(u1,u2,u3) * u1 * dot(grad(u3), grad(v_1)) * dx  \
+ (u2 - u_n2) / dt * v_2 * dx \
+ Do * dot(grad(u2), grad(v_2)) * dx \
- f * (Oam - u2) * v_2 * dx + kc * u1 * v_2 * dx \
+ (u3 - u_n3) / dt * v_3 * dx \
+ Df * dot(grad(u3), grad(v_3)) * dx \
+ d_w * u1 * u3 * v_3 * dx
...
vtkfile_u1 = File('u1_food/u1.pvd')
vtkfile_u2 = File('u2_food/u2.pvd')
vtkfile_u3 = File('u3_food/u3.pvd')
